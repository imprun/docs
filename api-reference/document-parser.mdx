...
| `right_type` | `string` | 권리종류 분류키 (예: `"ownership"`, `"mortgage"`, `"lease"`) |
...
| right_type | 설명 | 예시 |
|------------|------|------|
| `ownership` | 소유권 | 소유권보존, 소유권이전 |
| `mortgage` | 근저당권 | 근저당권설정, 근저당권이전 |
| `lease` | 임차권 | 주택임차권, 임차권설정 |
| `jeonse` | 전세권 | 전세권설정 |
| `superficies` | 지상권 | 지상권설정 |
| `provisional` | 가등기 | 소유권이전청구권가등기 |
| `seizure` | 압류·가압류 | 압류, 가압류 |
